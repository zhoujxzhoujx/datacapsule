    ...
    def data(self):
         ...
         # if self.web_ready():
            try:
                ...
                # print("登录成功")
                #全国数据总览
                target = self.wait_find_element('data', 'alldata')
                ...
                # 全国数据总面积十佳排行二维数据
                target_two = self.wait_find_element('data', 'TwoDime')
                target_two.click()
                # 全国数据总面积十佳排行增长二维数据
                target3 = self.wait_find_element('data', 'TwoDimeIncrese')
                target3.click()
                # sleep(1)
                ...
                target.click()
                # 全国数据总面积十佳排行增长原始数据
                target4 = self.wait_find_element('data', 'AlldataIncrese')
                target4.click()
                sleep(1)
                ...
                target3 = self.wait_find_element('data', 'data')
                target3.click()
                sleep(3)
...
if __name__ == '__main__':
            ...
            dd = Login()
            loginResult=dd.login('18520836299', 'airlook1234')
            if (loginResult == True):
